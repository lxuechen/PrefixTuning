...
run:
    to generate running scripts:
        python -m gpt2.launchers.prefix_vs_full_061521 --mode "submit"
    to run local:
        python -m gpt2.launchers.prefix_vs_full_061521 --mode "local"
"""
...
def _get_command(
    ...
    if mode == Mode.submit:
        ...
        if nonprivate == "no":
            # @formatter:off
            train_dir = (
                f"/nlp/scr/lxuechen/prefixtune/date_0615"
...
def main(
    ...
    **kwargs,
    ...
    if mode == Mode.local:
        ...
        commands = "#!/bin/bash\n"
        for seed in seeds:
            for max_grad_norm in (0.1, 0.5, 1):
                for lr in (1e-3, 1e-4, 1e-5):
                    for noise_multiplier in (0.2, 0.5, 0.7, 1.0):
        ...
        script_path = os.path.join('.', 'gpt2', 'scripts', f'prefix_vs_full_061521.sh')
