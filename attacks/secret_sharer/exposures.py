...
def compute_exposure_interpolation(
    ...
    perplexities_reference: List[float]
) -> Dict[int, List[float]]:
    """Get exposure using interpolation.

    "Approximation by sampling" in Section 4.3.
    ...
    perplexities_concat = np.concatenate([perplexities[r] for r in repetitions] + [perplexities_reference])
...
def compute_exposure_extrapolation(
    ...
    "Approximation by distribution model" in Section 4.3.
