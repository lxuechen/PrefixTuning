...
run:
    to generate running scripts:
        python -m gpt2stuff.launchers.prefix_vs_full_071821_v2 --mode "submit"
    to run local:
        python -m gpt2stuff.launchers.prefix_vs_full_071821_v2 --mode "local"
"""
...
def main(
    ...
    **kwargs,
    ...
    if mode == Mode.local:
        ...
        for seed in seeds:
            for max_grad_norm in (0.1,):
                for model_name_or_path in ("distilgpt2",):
                    for i, train_batch_size in enumerate((5,)):
                        for lr in (5e-5,):
                            for target_epsilon in (2,):
                                for epochs in (5,):
                                    for tuning_mode in ("lasttune",):
        ...
        script_path = os.path.join('.', 'gpt2stuff', 'scripts', f'prefix_vs_full_071821_v2.sh')
