...
date:
    0721
purpose:
    Gather E2E new results for gpt2-small fine-tuning.
notes:
    These are CPU-only jobs. So submit to john.
run:
    python -m gpt2stuff.launchers.gather_072121 --mode "submit"
    python -m gpt2stuff.launchers.gather_072121 --mode "local"
"""
...
def main(
    ...
    if mode == wrapper.Mode.local:
                        ...
                        for lr in (5e-4,):
                            for target_epsilon in (3, 5, 8, 2):
                                for tuning_mode in ("fulltune", "scratchtune", "prefixtune", "lineartune"):
                                    ...
                                    # @formatter:off
                                    train_dir = (
                                        f"/nlp/scr/lxuechen/prefixtune/date_0721"
        ...
        script_path = os.path.join('.', 'gpt2stuff', 'scripts', f'gather_072121.sh')
