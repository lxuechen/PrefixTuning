...
purpose:
    Gather all results
    - small batch size 0629
notes:
    These are CPU-only jobs. So submit to john.
run:
    python -m gpt2stuff.launchers.gather_062921 --mode "submit"
    python -m gpt2stuff.launchers.gather_062921 --mode "local"
"""
...
def main(
    ...
    if mode == wrapper.Mode.local:
                ...
                for lr in (5e-4, 1e-4, 5e-3, 1e-3):
                    for epochs in (60,):
                        for train_batch_size in (5, 25, 50, 100):
                            ...
                            # @formatter:off
                            train_dir = (
                                f"/nlp/scr/lxuechen/prefixtune/date_0629"
        ...
        script_path = os.path.join('.', 'gpt2stuff', 'scripts', f'gather_062921.sh')
