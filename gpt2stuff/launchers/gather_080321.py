...
date:
    0803
purpose:
    Gather new results for batch size saturation.
notes:
    These are CPU-only jobs. So submit to john.
run:
    python -m gpt2stuff.launchers.gather_080321 --mode "submit"
    python -m gpt2stuff.launchers.gather_080321 --mode "local"
"""
...
def main(
    ...
    if mode == wrapper.Mode.local:
        ...
        script_path = os.path.join('.', 'gpt2stuff', 'scripts', f'gather_080321.sh')
