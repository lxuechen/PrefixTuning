...
purpose:
    ...
    NO LR DECAY result in `date_0714`.
notes:
    These are CPU-only jobs. So submit to john.
run:
    python -m gpt2stuff.launchers.gather_071221 --mode "submit"
    python -m gpt2stuff.launchers.gather_071221 --mode "local"
"""
...
def main(
    ...
    if mode == wrapper.Mode.local:
                    ...
                    for target_epsilon in (3,):
                        for epochs in (50,):
                            for tuning_mode in ("fulltune",):
                                ...
                                # @formatter:off
                                train_dir = (
                                    f"/nlp/scr/lxuechen/prefixtune/date_0714"
        ...
        script_path = os.path.join('.', 'gpt2stuff', 'scripts', f'gather_071221.sh')
