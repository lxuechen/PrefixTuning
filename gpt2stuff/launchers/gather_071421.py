...
run:
    python -m gpt2stuff.launchers.gather_071421 --mode "submit"
    python -m gpt2stuff.launchers.gather_071421 --mode "local"
"""
...
def main(
    ...
    if mode == wrapper.Mode.local:
                            ...
                            for tuning_mode in ("fulltune",):
                                for lr in (1e-3, 5e-4, 1e-5, 5e-6):
                                    for max_grad_norm in (0.1,):
                                        ...
                                        # @formatter:off
                                        train_dir = (
                                            f"/nlp/scr/lxuechen/prefixtune/date_0713"
        ...
        script_path = os.path.join('.', 'gpt2stuff', 'scripts', f'gather_071421.sh')
